...
_iealwn.load = function() {
  var css =
    ...
    #iealertwn .iealwn-wrap{position:absolute;z-index:2147483647;top:50px;left:0;right:0;width:100%}\
  ...
  for (var i = 0; i < browsers.length; i++) {
    ...
    if (isXP && slug.match(/(chrome|firefox|edge)/g)) {
      ...
      var warningTitle =
        browsers[i]['name'] + '已经全面停止支持 Windows XP 系统！请选择其他浏览器。';
      ...
      browsersHtml +=
        ...
        slug +
        '_gray.png)">\
                    <a href="#" onclick="alert(\'Microsoft Edge 专为 Windows 10 打造！请选择其他浏览器或IE11。\');return false;" class="iealwn-browser-link" style="color:#777">' +
        browsers[i]['name'] +
        '</a>\
      ...
      browsersHtml +=
        ...
        '.png)">\
                    <a href="' +
        browsers[i]['url'] +
        '" class="iealwn-browser-link" target="_blank">' +
        browsers[i]['name'] +
        '</a>\
                    <a href="' +
        browsers[i]['durl'] +
        '" class="iealwn-browser-dlink" target="_blank" title="点击下载' +
        browsers[i]['name'] +
        '">下载</a>\
                </div>';
    }
  ...
  for (var browser in _iealwn.browserNames) {
    currentBrowserHtml +=
      '<a href="javascript:;" onclick="_iealwn.browserAlert(\'' +
      browser +
      '\')">' +
  ...
  currentBrowserHtml +=
    ' 其中一个吗？若是如此，您暂可不必升级，点击名称查看教程并按步骤切换至极速内核也可正常访问。</p>';
  ...
  var alertHtml =
    '<div id="iealertwn"><div class="iealwn-wrap"><div class="iealwn-box"><div class="iealwn-main">\
        ...
        <div class="iealwn-notice">\
            <div class="iealwn-title iealwn-title_alert">危险！您正在使用的 IE' +
    (_iealwn.iever ? _iealwn.iever : '') +
    ' 浏览器已过期，请立即升级！</div>\
            ...
            <div id="_iealwn_browserAlert">\
                ...
                <p>如您坚持使用当前浏览器访问本站，你将看到排版错误、功能不全、无法正常使用的网页，甚至是满屏乱码。请至少升级至 IE' +
    (_iealwn.outver + 1) +
    ' 或使用更先进的浏览器再访问。</p>\
            </div>\
            <div id="_iealwn_browserAlert_new">' +
    currentBrowserHtml +
    '</div>\
  ...
  if (!!names[browser]) {
    ...
    alertHtml +=
      ...
      '吗？</div>\
            <p>您正在使用的可能是' +
      browserName +
      ' IE 兼容模式。若是如此，您暂可不必升级，按下图所示步骤切换至极速内核也可正常访问。（示意图仅供参考，浏览器不同版本可能有差异）</p>\
            <p>如图片未显示，请在下方灰色区域点击右键选择“显示图片(H)”。</p>\
            <p><img class="iealwn-guide" src="' +
      _iealwn.dir +
      '/images/switch-' +
      browser +
      ...
      imgHeight +
      'px !important" alt="示意图" title="如图片未显示，请点击右键选择 显示图片(H)"></p>\
        ';
